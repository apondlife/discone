...
public sealed class BubblegumCharacterMusic: CharacterMusicBase {
    ...
    [SerializeField] Musicker.FmodMusicSource m_Source;
    ...
    void PlayStep() {
        ...
        // find line to play
        if (m_StepIdx % 2 == 0) {
            m_Source.PlayLine(m_FootstepsBass.Value, 0.0f, m_Key, extraParams: CurrentFmodParams);
        } else {
            var melody = m_FootstepsMelodies[m_MelodyIdx];
            m_Source.PlayNote(melody.Value[m_StepIdx / 2],0.0f, m_Key, extraParams: CurrentFmodParams);
        }
    ...
    /// play jump audio
    void PlayJump() {
        FMODPlayer.PlayAll(m_Source.PlayProgression(
            m_Jump.Value,
            m_JumpInterval,
            m_Key,
            extraParams: CurrentFmodParams
        ));
    }
    ...
    void PlayFlutter() {
        ...
        FMODPlayer.PlayEvent(m_Source.PlayLine(m_Flutter.Value, 0.0f, m_Key, extraParams: CurrentFmodParams));
